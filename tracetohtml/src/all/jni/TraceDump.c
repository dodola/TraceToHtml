...
typedef struct UniqueMethodEntry {
    ...
    int numMethods;
    struct MethodEntry **methods;
    /* list of methods with same name */
    int numCalls[2];                    /* 0=normal, 1=recursive */
} UniqueMethodEntry;
...
typedef struct MethodEntry {
    ...
    uint64_t elapsedExclusive;
    uint64_t elapsedInclusive;
    uint64_t topExclusive;
    /* non-recursive exclusive time */
    uint64_t recursiveInclusive;
    struct TimedMethod *parents[2];
    /* 0=normal, 1=recursive */
    struct TimedMethod *children[2];
    /* 0=normal, 1=recursive */
    int numCalls[2];
    /* 0=normal, 1=recursive */
    int index;
    /* used after sorting to number methods */
    int recursiveEntries;
...
void printClassProfiles(TraceData *traceData, uint64_t sumThreadTime) {
    ...
    for (ii = 0; ii < traceData->numClasses; ++ii) {
        ...
        if (gOptions.outputHtml) {
            ...
            className = htmlEscape(className, classBuf, HTML_BUFSIZE);
            printf("<div class=\"link\" onClick=\"javascript:toggle('d%d')\" onMouseOver=\"javascript:onMouseOver(this)\" onMouseOut=\"javascript:onMouseOut(this)\"><span class=\"parent\" id=\"xd%d\">+</span>",
                   ii, ii);
...
void printMethodProfiles(TraceData *traceData, uint64_t sumThreadTime) {
    ...
    for (ii = 0; ii < traceData->numUniqueMethods; ++ii) {
        ...
        if (gOptions.outputHtml) {
            ...
            methodName = htmlEscape(methodName, methodBuf, HTML_BUFSIZE);
            printf("<div class=\"link\" onClick=\"javascript:toggle('e%d')\" onMouseOver=\"javascript:onMouseOver(this)\" onMouseOut=\"javascript:onMouseOut(this)\"><span class=\"parent\" id=\"xe%d\">+</span>",
                   ii, ii);
 ...
 */
void profileTrace(TraceData *traceData, MethodEntry **pMethods, int numMethods,
                  uint64_t sumThreadTime) {
...
int usage(const char *program) {
    ...
    fprintf(stderr,
            "usage: %s [-ho] [-s sortable] [-d trace-file-name] [-g outfile] trace-file-name\n",
            program);
...
Java_dodola_watcher_utils_TraceUtils_analysisTraceFile(JNIEnv *env, jclass type, jstring path_) {
    const char *path = (*env)->GetStringUTFChars(env, path_, 0);
    gOptions.outputHtml = 1;//默认打开html输出

    char *end = ".html";
    char *outputPath = (char *) malloc(1 + strlen(path) + strlen(end));//..
    strcpy(outputPath, path);
    strcat(outputPath, end);

    //查看output文件是否存在,存在即删除
    int status = remove(outputPath);

    //重定向流
    if (freopen(outputPath, "w", stdout) == NULL)//error("重定向出错");
    {
        return -1;
    }

    gOptions.traceFileName = path;
    gOptions.threshold = 20;
